...
function lt_sc_products_deal( $atts, $content = null ){
    ...
    if (!isset($lt_opt['enable_optimized_speed']) || $lt_opt['enable_optimized_speed'] == 1) {
        ...
        if (isset($atts['is_ajax']) && $atts['is_ajax'] == 'yes' &&
            ...
            return lt_shortcode_text('lt_products_deal', $atts);
        }
...
function lt_add_to_cart_button_sc($product, $quickview = true){
    ...
    if($quickview){
        echo '<div class="add-to-cart-btn"><a href="javascript:void(0);" class="button small quick-view" data-prod="' . esc_attr($product->get_id()) . '" data-head_type="'.esc_attr($head_type).'">' . esc_html__('Shop now', 'lee_framework') . '</a></div>';
        return;
    }
        ...
        sprintf('
            ...
            ($product->is_purchasable() && $product->is_in_stock() && $product_type == 'simple') ? 
                'ajax_add_to_cart add_to_cart_button' : 
                    (($product_type == 'variable') ? 'ajax_add_to_cart_variable' : ''),
            esc_attr($product_type),
            esc_attr($head_type),
            esc_html($product->add_to_cart_text()),
            ($product_type == 'variable') ? '<a class="hidden-tag quick-view" data-prod="'.esc_attr($product->get_id()).'" data-head_type="'.esc_attr($head_type).'"></a>' : ''
        ),
    $product);
}
