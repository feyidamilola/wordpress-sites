    ...
    function lt_bery_VC_setup() {
        ...
        vc_map( array(
            ...
            "params" => array(
                ...
                array(
                    "type" => "dropdown",
                    "heading" => esc_html__("Optimized speed", 'lee_framework'),
                    "param_name" => "is_ajax",
                    "value" => array(
                        esc_html__('Yes', 'lee_framework') => 'yes',
                        esc_html__('No', 'lee_framework') => 'no'
                    ),
                    "std" => 'yes',
                    "admin_label" => true
                ),
                ...
                array(
                    "type" => "textfield",
                    "heading" => esc_html__('Min height (px)', 'lee_framework'),
                    "param_name" => "min_height",
                    "std" => 'auto',
                    "description" => esc_html__('Only use when Optimized speed "Yes"', 'lee_framework')
                ),
        ...
        vc_map( array(
            ...
            "params" => array(
                ...
                array(
                    "type" => "dropdown",
                    "heading" => __("Columns number tablet", 'lee_framework'),
                ...
                array(
                    "type" => "dropdown",
                    "heading" => __("Columns number small", 'lee_framework'),
        ...
        $service_box_params = array(
            ...
            "params" => array(
                ...
                array(
                    "type" => "textfield",
                    "heading" => __("Service Link", 'lee_framework'),
                    "param_name" => "service_link",
                    "std" => '#',
                    "description" => __("Enter service Link.", 'lee_framework'),
                ),
                array(
                    "type" => "textfield",
    ...
    function vc_getListProductDeals(){
        ...
        if ( $products->have_posts() ){
            while ( $products->have_posts() ) {
                ...
                $list[get_the_title()] = $product->get_id();
            }
        }
...
function lt_shortcode_vars($atts) {
    $variables = array();
    if (!empty($atts)) {
        foreach ($atts as $value) {
            $value = explode('=', $value);
            if (count($value) == 2) {
                $variables[$value[0]] = str_replace('"', '', $value[1]);
            }
        }
    }
...
function lt_shortcode_text($name = '', $atts = array(), $content = '') {
    global $id_shortcode;
    $GLOBALS['id_shortcode'] = !$id_shortcode ? 1 : $id_shortcode + 1;
    ...
    $result = '<div class="lt_load_ajax" data-id="' . $id_shortcode . '" id="ajax_lt_sc_' . $id_shortcode . '" data-shortcode="' . $name . '" style="min-height: ' . $height . '">';
    $result .= '<div class="please-wait type2" style="top:45%"></div>';
    $result .= '<div class="lt-shortcode-content hidden-tag">[' . $name;
...
/* Shortcode load Ajax */
add_action('wp_ajax_lt_load_ajax', 'lt_load_sc_ajax');
add_action('wp_ajax_nopriv_lt_load_ajax', 'lt_load_sc_ajax');
function lt_load_sc_ajax() {
    ...
    $result = array();
    foreach ($_POST['shortcode'] as $key => $shortcode) {
...
// shortcode ajax Load
add_action('wp_ajax_lt_load_ajax_shortcode', 'lt_ajax_load_shortcode');
add_action('wp_ajax_nopriv_lt_load_ajax_shortcode', 'lt_ajax_load_shortcode');
function lt_ajax_load_shortcode(){
    if (
        !isset($_POST['shortcode']) ||
        empty($_POST['shortcode']) ||
        !isset($_POST['shortcode_name']) ||
        empty($_POST['shortcode_name'])
    ) {
        die();
    }
    ...
    $result = shortcode_exists($_POST['shortcode_name']) ? do_shortcode($_POST['shortcode']) : '';
