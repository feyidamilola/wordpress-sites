...
if (!function_exists('lt_mini_cart')) {
    ...
    function lt_mini_cart($mini_cart_type = 'full', $echo = true) {
        ...
        $content = '<div class="mini-cart cart-inner mini-cart-type-' . $mini_cart_type . ' inline-block">' .
...
function lt_single_add_to_cart() {
    ...
    $product_variable = $product_type == 'variable' ? true : false;
        ...
        case 'large':
            $result .=
                    apply_filters('woocommerce_loop_add_to_cart_link', sprintf('
                        ...
                        </div>', esc_url($product->add_to_cart_url()), //link
                            esc_attr($product->get_id()), //product id
                            ($product->is_purchasable() && $product->is_in_stock() && $product_type == 'simple') ?
                                    'ajax_add_to_cart add_to_cart_button' : '', //class name
                            esc_attr($product_type), //product type
                            esc_html($head_type), esc_html($product->add_to_cart_text()) //add to cart text
                    ), $product);
            break;
        case 'small':
        default:
            $result .=
                    apply_filters('woocommerce_loop_add_to_cart_link', sprintf('
                        ...
                        </div>', esc_html($product->add_to_cart_text()), //data tip
                            esc_url($product->add_to_cart_url()), //link
                            esc_attr(isset($quantity) ? $quantity : 1 ), //link
                            esc_attr($product->get_id()), //product id
                            esc_attr($product->get_sku()), //product sku
                            ($product->is_purchasable() && $product->is_in_stock() && $product_type == 'simple') ? 'ajax_add_to_cart add_to_cart_button' : '', //class name
                            esc_attr($product_type), //product type
                            esc_html($head_type), esc_html($product->add_to_cart_text()) //add to cart text
                    ), $product);
            break;
...
// ! Wishlist link
// **********************************************************************//
if (!function_exists('lt_icon_wishlist')):

    function lt_icon_wishlist($echo = false) {
        if (!lt_has_woocommerce() || !defined('YITH_WCWL')) {
            return;
        }
        ...
        $tini_wishlist .= '<a href="' . esc_url($wishlist_page) . '" title="' . esc_html__('Wishlist', 'altotheme') . '">' .
                '<span class="pe7-icon pe-7s-like"></span>' .
                '<span class="wishlist-number' . $hasEmpty . '"><span class="lt-sl">' . (int) $count . '</span></span>' .
                esc_html__('Wishlist', 'altotheme') .
                '</a>';
...
function lt_products_page() {
    ...
    $compare_ver_3 = version_compare($woocommerce->version, '3.0.0', ">=");
    ...
    $visibility = !$compare_ver_3 ? 'catalog' : false;
    $hasResults = true;
    ...
    if ($term_id > 0) {
        ...
        $visibility = $visibility ? 'search' : $visibility;
    ...
    if ($_GET['hasPrice'] && ($_GET['min_price'] || $_GET['min_price'])) {
        if ($compare_ver) {
            ...
            if ($compare_ver_3) {
                $meta_price = wc_get_min_max_price_meta_query(array('min_price' => $min, 'max_price' => $max));
    ...
    if ($wp_query->post_count) {
        while ($wp_query->have_posts()) {
            $wp_query->the_post();
            wc_get_template('content-product.php', array('_delay' => $_delay, 'wrapper' => 'li', 'ajax' => true));
            $_delay += $_delay_item;
            $count++;
        }
        wp_reset_postdata();
    ...
    $json['description'] = lt_term_description($term_id, $type_taxonomy);
...
$total = 1, $current = 1, $type = 'list', $prev_text = 'PREV', $next_text = 'NEXT', $end_size = 3, $mid_size = 3, $prev_next = true, $show_all = false
) {
...
if (!function_exists('lt_term_description')) :

    function lt_term_description($term_id, $type_taxonomy) {
        if (!lt_has_woocommerce()) {
            return '';
        }
