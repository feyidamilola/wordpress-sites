...
"use strict";
$(window).stellar();
if($('body #lt-before-load').length > 0) {
...
jRes.addFunc({
    ...
    enter: function() {
        jPM.on();
        $('#jPanelMenu-menu').addClass('lt-menu-accordion');
        $('#jPanelMenu-menu .nav-dropdown').attr('class', 'nav-dropdown-mobile');
        $('#jPanelMenu-menu .nav-column-links').addClass('nav-dropdown-mobile');
        $('#jPanelMenu-menu').find('hr.hr-lt-megamenu').remove();
        $('#jPanelMenu-menu.lt-menu-accordion li').each(function(k, v){
    ...
    exit: function() {
        ...
        $('#jPanelMenu-menu.lt-menu-accordion li').each(function(){
            if($(this).hasClass('menu-item-has-children')){
                ...
                if(_shortcode && _shortcode_name) {
                    $.ajax({
                        ...
                        success: function(res){
                            ...
                            loadCountDown($);
                            loadToltip($);
                            var lt_deal = $(currentTab).find('.lt-sc-pdeal-block');
                            if($(lt_deal).length > 0){
                                var _id = $(lt_deal).attr('data-id');
...
if($('.wishlist-number').length > 0){
    ...
    $('body').on('click', '.btn-wishlist', function(){
        $(this).ajaxSuccess(function(event, request, settings) {
            ...
            if(typeof request === 'object'){
                var strfirst = request.responseText.substring(0, 1);
                if(strfirst === '{'){
                    var data = JSON.parse(request.responseText);
                    if(data.result !== undefined){
                        if(data.result === 'true' && $('.wishlist-number .lt-sl').length && !_flag_add){
                            _flag_add = true;
                            var sl_wislist = parseInt($('.wishlist-number .lt-sl').text());
                            sl_wislist = sl_wislist ? (sl_wislist + 1) : 1;
                            $('.wishlist-number .lt-sl').html(sl_wislist);
                            if (sl_wislist > 0) {
                                $('.wishlist-number').removeClass('lt-product-empty');
                            } else if(sl_wislist == 0 && !$('.wishlist-number').hasClass('lt-product-empty')){
                                $('.wishlist-number').addClass('lt-product-empty');
                            }
                        }
                    }
                }
    ...
    var _flag_remove = false;
    $('body').on('click', '.remove_from_wishlist', function(){
        $('body').ajaxSuccess(function() {
            if($('.wishlist-number .lt-sl').length && !_flag_remove){
                _flag_remove = true;
                var sl_wislist = parseInt($('.wishlist-number .lt-sl').text());
                    sl_wislist = sl_wislist ? (sl_wislist - 1) : 0;
                $('.wishlist-number .lt-sl').html(sl_wislist);
                
                if(sl_wislist == 0 && !$('.wishlist-number').hasClass('lt-product-empty')){
                    $('.wishlist-number').addClass('lt-product-empty');
                }
            }
        }).ajaxComplete(function(){
...
$('body').on('click', '.toggle-sidebar, .cart-link', function(){
...
$('body').on('click', '.lt_add_to_cart', function(){
    ...
    if(_id){
        ...
        if(($(_this).hasClass('lt_add_to_cart_variation') || _type == 'variations') && !_variation_id){
            return false;
        }else{
    ...
    if($('.lt-has-filter-ajax').length < 1) {
        ...
        if(!$(this).hasClass('lt-disable') && !$(this).hasClass('lt-active')){
            ...
            if(_catid){
                ...
                lt_Ajax_filter($, ajaxurl, _url, _page, _catid, _order, _variations, hasPrice, min, max,  0, '', _this, '1', _taxonomy);
            }
        }
        return false;
    }
...
$('body').on('change', 'select[name="orderby"]', function(){
    if($('.lt-has-filter-ajax').length < 0) {
        ...
        var _hasSearch = ($('input#lt_hasSearch').length > 0 && $('input#lt_hasSearch').val() !== '') ? 1 : 0;
        var _s = (_hasSearch === 1) ? $('input#lt_hasSearch').val() : '';
        
        lt_Ajax_filter($, ajaxurl, _url, _page, _catid, _order, _variations, hasPrice, min, max, _hasSearch, _s, _this, '0', _taxonomy);
        return false;
    }
});
...
if($(".price_slider").length){
    ...
    $('body').on('click', '.reset_price', function(){
        if($('.lt_hasPrice').length && $('.lt_hasPrice').val() === '1'){
...
function lt_Ajax_filter($, ajaxurl, _url, _page, _catid, _order, _variations, _hasPrice, _min, _max, _hasSearch, _s, _this, _top, _taxonomy){
    $.ajax({
        url: ajaxurl,
        type: 'get',
        dataType: 'json',
        ...
        success: function(res){
            ...
            $('#lt-hidden-current-cat').attr('data-taxonomy', _taxonomy);
            ...
            var pagestring = '';
            if (_page) {
                if (_h || (_hasSearch == 1) || (res.friendly == 0)) {
            ...
            $('li.cat-item-' + _catid + ' > .lt-filter-by-cat').addClass('lt-active');
            $('.tag-link-' + _catid).addClass('lt-active');
...
function loadingSCCarosel($){
    if($('.lt-sc-carousel').length){
        $('.lt-sc-carousel').each(function(){
            var _sc = $(this);
...
function reponsiveBanners($) {
    ...
    if($('.banner-image').length > 0) {
        ...
        $('.banner-image').each(function() {
            ...
            if(_wH < 946) {
                var _resize = _wH / 1200;
                $(_parent).height(_defH * _resize * _zoom);
                $(_parent).find('.banner-content').css({'font-size': (_zoom * 100 * _resize).toString() + '%'});
            } else {
                $(_parent).height(_defH);
                $(_parent).find('.banner-content').css({'font-size': '100%'});
            }
        });
    }
}