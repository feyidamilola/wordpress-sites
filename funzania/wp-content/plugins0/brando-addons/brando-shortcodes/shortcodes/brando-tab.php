...
if ( ! function_exists( 'brando_tabs' ) ) {
  function brando_tabs( $atts, $content = null ) {
    ...
    ( $brando_title_color ) ? $tz_featured_array[] = '.'.$brando_token_class.' .nav-tabs li a,.'.$brando_token_class.' .nav-tabs>li.active>a:focus, .'.$brando_token_class.' .nav-tabs li a i, .'.$brando_token_class.' .nav-tabs li:active a i { color:'.$brando_title_color.' !important;}' : '';
    ( $brando_border_color ) ? $tz_featured_array[] = '.'.$brando_token_class.' .nav-tabs li.active a, .'.$brando_token_class.' .nav-tabs li a:hover, .'.$brando_token_class.' .nav-tabs li a:focus, .'.$brando_token_class.' .nav-tabs>li.active>a:focus { border-bottom-color:'.$brando_border_color.';}' : '';

    //Font Settings For Title
    $fontsettings_title_class = $fontsettings_title_id = $responsive_style = '';
    print_r($title_settings);
        ...
        $output .= '<div'.$id.' class="col-md-4 col-sm-12 col-xs-12'.$class.' '.$brando_token_class.'">';
          $output .= '<ul class="nav nav-tabs '.$tabs_style.' no-border alt-font text-large'.$tabs_alignment.'" role="tablist">';
            foreach( $brando_global_tabs as $key => $tab) {
              ...
              $img_alt_etline = brando_option_image_alt($custom_icon_image);
              $img_title_etline = brando_option_image_title($custom_icon_image);
              ...
              $output .= '<li role="presentation" class="sm-no-margin '.$active.'">';
                $output .= '<a href="#brando-'.$tabuniqtab.'-'.$key.'" role="tab" data-toggle="tab" class="'.$fontsettings_title_class.'">';
                  if( $custom_icon == 1 && !empty( $custom_image[0] ) ) {
          ...
          $output .= '<ul class="nav nav-tabs margin-ten-bottom xs-margin-six-bottom'.$tabs_alignment.'">';
            foreach( $brando_global_tabs as $key => $tab) {
              ...
              $output .= '<a href="#brando-'.$tabuniqtab.'-'.$key.'" data-toggle="tab" class="'.$fontsettings_title_class.'">';
                if( $custom_icon == 1 && !empty( $custom_image[0] ) ) {
                      $output .= '<span><img src="'.$custom_image[0].'"'.$image_alt_etline.$image_title_etline.' class="icon-image" width="'.$custom_image[1].'" height="'.$custom_image[2].'"'.$srcset_data_icon.$sizes_data_icon.' /></span>';
                }elseif($tab_icon){
