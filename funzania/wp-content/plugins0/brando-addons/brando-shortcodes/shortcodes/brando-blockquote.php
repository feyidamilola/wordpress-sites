...
if ( ! function_exists( 'brando_blockquote_shortcode' ) ) {
	function brando_blockquote_shortcode( $atts, $content = null ) {
		extract( shortcode_atts( array(
			    ...
			    'brando_blockquote_icon_color' => '',
			    'brando_token_class' => '',
			    'title_settings' => ''
	        ), $atts ) );

		global $tz_featured_array,$font_settings_array;
		$output = $style_footer_atrr = $style_atrr = $fontsettings_title_class = $fontsettings_title_id = '';
	    ...
	    ( $brando_blockquote_icon_color ) ? $tz_featured_array[] ='.'.$brando_token_class.' blockquote p::before{ color:'.$brando_blockquote_icon_color.';}' : '';
		...
		$output .= '<div'.$id.$class_attr.'>';
	        $output .= '<blockquote class="alt-font"'.$style_atrr.'>';
	            ...
	            $output .= '<footer class="text-uppercase black-text '.$fontsettings_title_class.'"'.$style_footer_atrr.'>'.$brando_blockquote_heading.'</footer>';
	        $output .= '</blockquote>';
	    $output .= '</div>';
