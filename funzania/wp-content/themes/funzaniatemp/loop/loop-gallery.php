	...
	if(is_array($brando_gallery)){
		foreach ($brando_gallery as $key => $value) {
			...
			if($brando_thumb[0]){
				...
				$sizes = $sizes_data = '';
			    $sizes = wp_get_attachment_image_sizes( $value, $brando_srcset_data );
			    if( $sizes ){
			        $sizes_data = ' sizes="'.esc_attr( $sizes ).'"';
			    }
                echo '<div class="col-md-4 col-sm-6 col-xs-12 no-padding">';
                    echo '<a class="lightboxgalleryitem" data-group="'.esc_attr($brando_popup_id).'" '.$brando_image_lightbox_title.$brando_image_lightbox_caption.' href="'.$brando_full_image[0].'"><img src="'.esc_url($brando_thumb[0]).'" width="'.$brando_thumb[1].'" height="'.$brando_thumb[2].'" '.$brando_image_alt.$brando_image_title.$srcset_data.$sizes_data.'/></a>';
                echo '</div>';
            }
		}
	}
