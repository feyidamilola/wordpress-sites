		    ...
		    echo '<div class="wrap">';
		        ...
		        echo '<div class="brando-header-licence">';
		            ...
		            echo '<div class="brando-head-right">';
		                echo '<a target="_blank" href="'.$brando_theme->get( 'ThemeURI' ).'/documentation/">'.esc_html__( 'Online Documentation', 'brando' ).'</a><span class="link_sep">|</span><a target="_blank" href="'.$brando_theme->get( 'AuthorURI' ).'/support.php">'.esc_html__( 'Support Center', 'brando' ).'</a></div>';
			        ...
			        echo '<div class="licence-content-top">';
                        ...
                        echo '<div class="licence-content-bottom">';    
                            ...
                            if( $brando_is_theme_licence_active ) {
                                echo '<div class="licence-activated-success"><i class="fa fa-check-circle"></i><span>'.esc_html__( 'Awesome! Your Brando WordPress theme license is activated already. Enjoy premium features of Brando.', 'brando' ).'</span></div>';
                                    ...
                                    $brando_get_transient = get_transient( 'brando_licence_token' );
                                   	if( $_GET['token'] == $brando_get_transient ) {
                                        if( $_GET['response'] == 'true' && isset( $_GET['msg']) ) {
                                           	echo '<div class="licence-activated-success"><i class="fa fa-check-circle"></i><span>'.esc_attr( $_GET['msg'] ).'</span></div>';
                                                $class = ' hide-licence-button"';
                                                brando_theme_active_licence( 'yes' );
                                        }
                                        if( $_GET['response'] == 'false' && isset( $_GET['msg']) ) {
                                          	echo '<div class="licence-activated-failed"><i class="fa fa-times-circle"></i><span>'.esc_attr( $_GET['msg'] ).'</span></div>';
                                        }
                                        if( $_GET['response'] == 'access_denied' && isset( $_GET['msg']) ) {
                                          	echo '<div class="licence-activated-access-denied"><i class="fa fa-info-circle"></i><span>'.esc_attr( $_GET['msg'] ).'</span></div>';
                                        }
                                    }
                                }
                            }
                        ...
                        echo '<div class="licence-support-content-bottom">';
                        	...
                        	echo '<div class="license-support">';
                        		echo '<a href="'.$brando_theme->get( 'AuthorURI' ).'/support.php" target="_blank"><img src="'.BRANDO_THEME_IMAGES_URI.'/support-center.png" /><span>'.esc_html__( 'Support Center','brando').'</span></a>';
                        	echo '</div>';
