	...
	if ( ! class_exists( 'CBCurrencyconverterSetting' ) ):
		...
		class CBCurrencyconverterSetting {
			...
			function callback_numberbutton( $args ) {
				...
				$html = sprintf( '<input type="number" style="height: 30px;" class="%1$s-text" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" step="0.000000001"/>', $size, $args['section'], $args['id'], $value );
					...
					<?php foreach ( $this->settings_sections as $form ) { ?>
						<div id="<?php echo $form['id']; ?>" class="group" style="display: none;">
							<form method="post" action="options.php">
								<?php
									do_action( 'cbcurrencyconverter_form_top_' . $form['id'], $form );
									settings_fields( $form['id'] );
									do_settings_sections( $form['id'] );
									do_action( 'cbcurrencyconverter_form_bottom_' . $form['id'], $form );
								?>
