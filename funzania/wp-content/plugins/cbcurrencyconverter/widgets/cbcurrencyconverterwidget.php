<?php
	...
	class CBCurrencyConverterWidget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			$cbcur_to_currency   = ( isset( $setting_api_cal['cbcurrencyconverter_tocurrency'] ) ) ? $setting_api_cal['cbcurrencyconverter_tocurrency'] : 'CAD';
			...
			if ( $layout == 'list' ) {
				echo CBCurrencyConverterHelper::cbxcclistview( 'widget', $instance );
			} elseif ( $layout == 'cal' ) {
				echo CBCurrencyConverterHelper::cbxcccalcview( 'widget', $instance );
			} elseif ( $layout == 'calwithlistbottom' ) {
				echo CBCurrencyConverterHelper::cbxcccalcview( 'widget', $instance ) . cbxcclistview( 'widget', $instance );
			} elseif ( $layout == 'calwithlisttop' ) {
				echo CBCurrencyConverterHelper::cbxcclistview( 'widget', $instance ) . cbxcccalcview( 'widget', $instance );
			}
		...
		public function form( $instance ) {
			...
			$instance = wp_parse_args( (array) $instance, array(
				'title'        => esc_html__( 'Currency Calculator', 'cbcurrencyconverter' ),
				//'hide'         => '',
				'decimalpoint' => 2,
				'layout'       => 'cal'
			) );
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'layout' ); ?>"><?php esc_html_e( 'Select Layout:', 'cbcurrencyconverter' ); ?>
					<select name="<?php echo $this->get_field_name( 'layout' ); ?>"
							id="<?php echo $this->get_field_id( 'layout' ); ?>" class="chosen-select widefat">
						<option
							value="cal" <?php selected( $instance['layout'], 'cal' ); ?>><?php esc_html_e( 'Calculator', 'cbcurrencyconverter' ); ?></option>
						<option
							value="list" <?php selected( $instance['layout'], 'list' ); ?>><?php esc_html_e( 'List', 'cbcurrencyconverter' ); ?></option>
						<option
							value="calwithlistbottom" <?php selected( $instance['layout'], 'calwithlistbottom' ); ?>><?php esc_html_e( 'Calc with List Bottom', 'cbcurrencyconverter' ); ?></option>
						<option
							value="calwithlisttop" <?php selected( $instance['layout'], 'calwithlisttop' ); ?>><?php esc_html_e( 'Calc with List Top', 'cbcurrencyconverter' ); ?></option>
					</select>
				</label>
			</p>
