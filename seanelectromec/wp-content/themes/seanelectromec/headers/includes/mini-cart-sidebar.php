        ...
        <div class="cart_list">
            ...
            foreach ($woocommerce->cart->cart_contents as $cart_item_key => $cart_item) :
                ...
                if ($_product->exists() && $cart_item['quantity'] > 0) :
                    $price = (int) $cart_item['quantity'] * wc_price($_product->get_price());
                    ?>  
                    <div class="row mini-cart-item collapse" id="item-<?php echo $_product->get_id(); ?>">
                        ...
                        <div class="small-7 large-7 columns">
                            <div class="mini-cart-info">
                                ...
                                echo '<div class="cart_list_product_quantity">' . $cart_item['quantity'] . ' x ' . wc_price($_product->get_price()) . '</div>';
                                ?>
                            </div>
                        </div>
                        <div class="small-2 large-2 columns text-right">
                            ...
                            echo apply_filters(
                                'woocommerce_cart_item_remove_link', sprintf(
                                    '<a href="javascript:void(0);" data-key="%s" data-id="%s" class="remove item-in-cart" title="%s"><i class="pe-7s-close-circle"></i></a>', $cart_item_key, $_product->get_id(), esc_html__('Remove this item', 'altotheme')
                                ), $cart_item_key
                            );
                            ?>
                        </div>
