...
if (!function_exists('lt_get_main_menu')){
    function lt_get_main_menu($main_menu_type = '1', $main = true){
    ...
    if (defined('DOING_AJAX') && DOING_AJAX && !empty($query->query_vars['s'])) {
        foreach ($posts as $key => $post) {
            foreach (array('myaccount', 'edit_address', 'change_password', 'lost_password', 'shop', 'cart', 'checkout', 'pay', 'view_order', 'thanks', 'terms') as $wc_page_type) {
                if( $post->ID == wc_get_page_id($wc_page_type) ) unset($posts[$key]);
            }
        }
    }
    return $posts;
...
add_action('lt_theme_before_load', 'lt_theme_before_load');
if (!function_exists('lt_theme_before_load')):

    function lt_theme_before_load() {
        global $lt_opt;

        if (isset($lt_opt['effect_before_load']) && $lt_opt['effect_before_load'] == 1) {
            echo 
            '<div id="lt-before-load">' .
                '<div class="lt-relative lt-center">' .
                    '<div class="please-wait type2"></div>' .
                '</div>' .
            '</div>';
        }
    }
