...
if ( $shop_page_id > 0 && strstr( $permalinks['product_base'], '/' . $shop_page->post_name ) && get_option( 'page_on_front' ) !== $shop_page_id ) {
    $prepend = $before . '<a href="' . get_permalink( $shop_page ) . '" class="lt-filter-by-cat" data-id="0">' . $shop_page->post_title . '</a> ' . $after . $delimiter;
}
...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
    ...
    if ( is_category() ) {
        ...
        $current_term = (!$ajax) ? 
            ...
            get_term_by( 'term_id', $_GET[ 'catId' ], get_query_var( 'taxonomy' ));
