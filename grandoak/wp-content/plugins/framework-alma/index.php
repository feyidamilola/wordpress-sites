...
if ( $my_theme->get('Author') == "PixelThrone") {
	...
	if ( getenv( 'APP_ENV' ) == 'local' ) {
		if ( ! defined( 'PLUGIN_PATH' ) ) {
			define( 'PLUGIN_PATH', WP_PLUGIN_DIR . '/framework-alma/' );
		}
		if ( ! defined( 'PLUGIN_URL' ) ) {
			define( 'PLUGIN_URL', plugins_url( 'framework-alma/' ) );
		}
	} else {
		if ( ! defined( 'PLUGIN_PATH' ) ) {
	...
	function alma__admin__redux_is_required()
		{
			$title = __('Warning', 'ALMA_THEME');
			$text1 = __( 'The', 'ALMA_THEME' );
			$text4 = __( 'Please install to activate the plugin.', 'ALMA_THEME' );
			$text5 = __( 'plugin requires the', 'ALMA_THEME' );

			echo
			<<<HTML
							<div class="error pt_ext__redux_is_required">
					<h3>$title</h3>
					<p>$text1 <strong>$text3</strong> $text5 <a href="/wp-admin/themes.php?page=tgmpa-install-plugins">Redux Framework</a>.<br> $text4 </p>
				</div>
HTML;
		}
	...
	function pt_plugin_override() {
		...
		update_option( 'hicpo_options', array('objects' => array('post', 'page', 'portfolio')) );
...
//			require_once(dirname(__FILE__). '/theme-options/ReduxCore/framework.php');
			add_action( 'admin_notices', 'alma__admin__redux_is_required' );
		}
