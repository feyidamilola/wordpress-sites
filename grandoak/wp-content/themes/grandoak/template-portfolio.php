...
if ( $query->have_posts() ) {
	...
	if ( get_field('portfolio_bg_color', $page_id) ) {
		...
		if ( $info_style == 'style_2' ) {
			...
			echo 'section.portfolio-page ul.portfolio-items li .info { background: '. $bg_rgba .' !important; }';
		}
			...
			echo '<div class="content">';
				...
				if ( isset($page_options['raw']['show_folio_categories']) && $page_options['raw']['show_folio_categories'] )
				{
					...
					if ( !isset($categories['errors']) ) :
						...
						{
							// var_dump($categorie);
							echo '<li><a href="#" data-filter=".filter--'. $categorie->term_id .'"><span>'. $categorie->name  .'</span></a></li>';
						}
					endif;
				...
				while ( $query->have_posts() ) {
					...
					if ( has_post_thumbnail() ) :
						...
						$portfolio_category = wp_get_post_terms( $post_id, 'portfolio_category', array("fields" => "all"));
						...
						if ( $portfolio_category && !is_wp_error( $portfolio_category ) ) :
							foreach ($portfolio_category as $row) {
								...
								// print_r($row);
								$categories .= $row->name.', ';
								$categories_slug .= 'filter--'.$row->term_id.' ';
							}
						endif;
